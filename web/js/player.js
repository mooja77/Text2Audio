const Player = {
  mount(container, item) {
    container.innerHTML = `
      <div class="player">
        <div style="font-weight:600;font-size:16px">${T2A.esc(item.title)}</div>
        <div class="muted" style="font-size:13px">${T2A.esc(item.author || "")} · ${item.chapters.length} chapters</div>
    ...
    chaps.innerHTML = item.chapters.map((c, i) =>
      `<div class="chap" data-s="${c.startMs / 1000}" data-i="${i}">
         <span class="muted">${i + 1}</span><span style="flex:1">${T2A.esc(c.title)}</span>
